...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        h, w = x.shape[-2:]
        x = self.body(to_3d(x))
        # print("to_3d x:{}".format(x.shape))
        x = to_4d(x, h, w)
        # print("to_4d x:{}".format(x.shape))
        return x
...
# 多头自注意力
class Mutual_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(Mutual_Self_Attention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.q = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.k = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
...
class ChannelAttentionTransformerBlock(nn.Module):   # 有归一化操作的通道注意力机制
    def __init__(self, dim, num_heads, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias'):
        super(ChannelAttentionTransformerBlock, self).__init__()

        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = Mutual_Self_Attention(dim, num_heads, bias)
        # mlp
        self.norm2 = nn.LayerNorm(dim)
        mlp_hidden_dim = int(dim * ffn_expansion_factor)
        self.ffn = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=0.)
    ...
    def forward(self, fusion):
        ...
        fused = fusion + self.attn(self.norm1(fusion)) # b, c, h, w
        # mlp
        fused = to_3d(fused) # b, h*w, c
        fused = fused + self.ffn(self.norm2(fused))
        fused = to_4d(fused, h, w)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 平均池化分支
        avg_out = self.fc(self.avg_pool(x).view(b, c))  # [B, C] → [B, C/r] → [B, C]
        # 最大池化分支
        max_out = self.fc(self.max_pool(x).view(b, c))  # [B, C] → [B, C/r] → [B, C]
        
        # 合并结果
        channel_weights = self.sigmoid(avg_out + max_out)  # [B, C]
        return channel_weights.view(b, c, 1, 1)  # 扩展维度 [B, C, 1, 1]
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 生成空间注意力图
        spatial_weights = self.conv(combined)  # [B, 1, H, W]
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x * channel_weights  # 广播乘法 [B, C, H, W]
        
        # 空间注意力
        spatial_weights = self.spatial_att(x)
        x = x * spatial_weights  # 广播乘法 [B, C, H, W]
        return x
