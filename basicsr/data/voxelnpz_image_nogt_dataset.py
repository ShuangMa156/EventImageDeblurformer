...
class VoxelnpzPngSingleDeblurDataset_nogt(data.Dataset):
    ...
    def __init__(self, opt):
        super(VoxelnpzPngSingleDeblurDataset_nogt, self).__init__()
        self.opt = opt
        ...
        assert len(blur_frames) == len(event_frames), f"Mismatch in blur ({len(blur_frames)}), sharp ({len(sharp_frames)}), and event ({len(event_frames)}) frame counts."
